...
export default function ShoppingCartPage() {
  ...
  const handleCheckout = async () => {
    ...
    completeOrder(paymentMethod === 'bank' ? `Bank (${selectedBank?.toUpperCase()})` : paymentMethod.toUpperCase());
                ...
                <div className="bg-[#0c0c0c] border border-gray-800 rounded-[32px] p-8 sticky top-32 shadow-2xl">
                  ...
                  <div className="py-6 border-y border-gray-900 my-6 space-y-3">
                    ...
                    <div className="flex justify-between items-baseline">
                      ...
                      <span className="text-3xl font-bold text-yellow-400 font-mono">${total.toFixed(2)}</span>
                    </div>
                  </div>

                  <div className="space-y-3 mb-8">
                    <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest block mb-2">Select Method</span>
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        onClick={() => setPaymentMethod('credit')}
                        className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 cursor-pointer ${paymentMethod === 'credit' ? 'border-yellow-400 bg-yellow-400/5 text-white' : 'border-gray-900 text-gray-600'
                          }`}
                      >
                      ...
                      <button
                        ...
                        className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 cursor-pointer ${paymentMethod === 'bank' ? 'border-yellow-400 bg-yellow-400/5 text-white' : 'border-gray-900 text-gray-600'
                          }`}
                      >
                    ...
                    {paymentMethod === 'bank' && (
                      <div className="p-4 bg-gray-900/50 rounded-2xl border border-gray-800 animate-in zoom-in-95 mt-4">
                        <p className="text-[10px] text-gray-500 mb-3 uppercase tracking-widest font-bold">Transfer to:</p>
                        <div className="flex justify-between items-center bg-black p-3 rounded-xl border border-gray-800">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 bg-blue-600 rounded flex items-center justify-center text-[10px] font-bold">BCA</div>
                            <span className="text-sm font-mono tracking-tighter">8831024552</span>
                          </div>
                          <button onClick={() => navigator.clipboard.writeText('8831024552')} className="text-[10px] text-yellow-400 font-bold hover:underline">COPY</button>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  >
                    ...
                    {isProcessing ? 'Authorizing...' : 'Pay and Complete'}
                  </button>
                </div>
              </div>
            )}
      ...
      {isSuccess && (
        <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-xl flex items-center justify-center p-4">
          <div className="max-w-md w-full text-center animate-in zoom-in-95 duration-300">
            ...
            <p className="text-gray-400 mb-10">Your collection has been updated with your new assets. Happy creating!</p>
